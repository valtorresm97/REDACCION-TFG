...
\begin{table}[H]
    ...
    \begin{tabular}{p{0.30\textwidth}p{0.22\textwidth}p{0.38\textwidth}}
        ...
        Snapshot & \SI{0.2}{\second}, \SI{5}{\hertz} & Publicación de la vista pública; no marca la cadencia musical. \\
        \hline
    \end{tabular}
\end{table}

Por tanto, el backend maneja varias cadencias simultáneas. Los bloques EEG llegan cada \SI{32}{\milli\second}; las características espectrales se recalculan aproximadamente cada \SI{256}{\milli\second}; y el \textit{snapshot} se reconstruye cada \SI{0.2}{\second} para WebUI, capturas y validación. Es decir, las características EEG y los controles internos de sonificación se actualizan nominalmente unas \SI{3.9}{\hertz}, no a \SI{5}{\hertz}. Esta última frecuencia solo corresponde a la publicación del estado; la generación musical sigue una cadencia propia, asociada a la duración del compás musical.
...
En la ruta en vivo se utiliza \texttt{psd\_method="multitaper"}. Antes de estimar la PSD, \texttt{DSPCore} aplica un preprocesado robusto sobre la ventana seleccionada. Después calcula la PSD, integra potencia por bandas y resume la ventana mediante un diccionario de \textit{features}. En la ruta de tiempo real no se publica el espectro completo, sino un conjunto compacto de variables preparado para ser consumido por el resto del sistema. La Tabla~\ref{tab:impl_backend_features} resume estas salidas sin listar cada clave de forma exhaustiva.

\begin{table}[H]
    \centering
    \caption{Resumen compacto de características espectrales generadas por el backend.}
    ...
    \begin{tabular}{p{0.30\textwidth}p{0.58\textwidth}}
        ...
        Uso posterior & Variables de entrada para calidad, WebUI, capturas y sonificación; no se emplean como indicadores clínicos. \\
        \hline
    \end{tabular}
\end{table}
...
\begin{table}[H]
    ...
    \begin{tabular}{p{0.24\textwidth}p{0.30\textwidth}p{0.36\textwidth}}
        ...
        \hline
        \texttt{features} & Contenido espectral de la ventana: RMS, picos y potencias por banda. & Alimentar la WebUI, las capturas y la sonificación. \\
        \texttt{diagnostics} & Estado observacional de la señal: amplitud, offset, saturación, señal plana, saltos y ruido de red. & Detectar problemas de adquisición sin modificar el DSP. \\
        \texttt{spectral\_quality} & Puntuación, estado y factor de puerta derivados de \texttt{features}, \texttt{diagnostics} y recepción. & Decidir si las features se usan con normalidad, atenuación o bloqueo. \\
        \hline
    \end{tabular}
\end{table}
...
\begin{table}[H]
    ...
    \begin{tabular}{p{0.28\textwidth}p{0.27\textwidth}p{0.35\textwidth}}
        \hline
        \textbf{Bloque} & \textbf{Estados} & \textbf{Interpretación} \\
        \hline
        \texttt{status} & \texttt{waiting\_for\_data}, \texttt{waiting\_for\_window} & El backend espera datos o acumula la ventana mínima de \SI{4}{\second}. \\
        \texttt{status} & \texttt{receiving}, \texttt{features\_ready} & La ventana existe y puede haber características disponibles para los consumidores. \\
        \texttt{spectral\_quality} & \texttt{clean}, \texttt{usable\_with\_caution} & La ventana puede usarse con normalidad o con penalización leve. \\
        \texttt{spectral\_quality} & \texttt{artifact\_suspected}, \texttt{bad} & La ventana requiere atenuación, congelación o bloqueo de la sonificación. \\
        \hline
    \end{tabular}
\end{table}
...
\begin{table}[H]
    ...
    \begin{tabular}{p{0.25\textwidth}p{0.31\textwidth}p{0.34\textwidth}}
        ...
        \hline
        \texttt{config/status} & Configuración activa y fase del runtime. & Indicar con qué parámetros trabaja el backend y si ya hay ventana o features. \\
        \texttt{rx} & Tasas de recepción, pérdidas, cola, bloques malformados y estados inválidos. & Vigilar la estabilidad del contrato MCU--Python. \\
        \texttt{features/diagnostics} & Resumen espectral y métricas observacionales de la ventana reciente. & Exponer el estado de la señal sin publicar el espectro completo. \\
        \texttt{spectral\_quality} & Puntuación, estado, factor de puerta y validez para sonificación. & Regular el uso de las features por módulos posteriores. \\
        \texttt{capture/performance/errors} & Estado de captura, cadencias principales y contadores de errores. & Facilitar validación experimental y depuración. \\
        \hline
    \end{tabular}
\end{table}
