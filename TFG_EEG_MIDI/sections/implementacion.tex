...
En la ruta en vivo se utiliza \texttt{psd\_method="multitaper"}. \texttt{DSPCore} preprocesa la ventana, estima la PSD, integra la potencia por bandas y devuelve un diccionario compacto de \textit{features}. No se publica el espectro completo en cada actualización: se exponen solo variables resumidas, suficientes para WebUI, capturas, calidad y sonificación. La Tabla~\ref{tab:impl_backend_features} agrupa estas salidas sin convertir la memoria en una documentación exhaustiva de la API interna.
...
Las bandas delta, theta, alpha, beta y gamma se usan como descriptores cuantitativos de la ventana reciente y como entrada algorítmica para módulos posteriores. Su uso en este TFG es funcional y experimental, no clínico.

Además de las \textit{features}, el procesador genera \texttt{diagnostics}, que resume el estado observacional de la señal: amplitud en microvoltios, offset, dispersión, saturación, señal plana, saltos bruscos y presencia relativa de energía en torno a \SI{50}{\hertz}. A partir de \texttt{features}, \texttt{diagnostics}, métricas de recepción y disponibilidad de ventana, \texttt{spectral\_quality.py} calcula una puntuación, un estado y un factor de puerta. Esta capa no recalcula la PSD ni altera la señal; solo decide cómo de fiable es la última ventana para alimentar los módulos posteriores.
...
\texttt{BackendService} coordina el ciclo de ejecución del backend: drena bloques desde \texttt{EEGReceiver}, actualiza el buffer temporal, calcula nuevas características cuando corresponde, evalúa la calidad espectral y publica periódicamente un \textit{snapshot}. Este \textit{snapshot} es la vista estructurada que consumen la WebUI, las herramientas de captura, los registros en disco y los módulos posteriores.

El estado principal del runtime se publica dentro de \texttt{status.state}. Este estado no indica calidad de señal, sino fase de ejecución. La Figura~\ref{fig:impl_backend_runtime_states} muestra esta diferencia frente a \texttt{spectral\_quality.state}, que resume la fiabilidad de la última ventana analizada.
...
La Tabla~\ref{tab:impl_backend_estados} resume los estados principales sin repetir la lógica de transición mostrada en la figura.
...
El \textit{snapshot} no introduce una etapa nueva de procesado: agrupa configuración, estado de ejecución, recepción, características, diagnósticos, calidad y datos auxiliares de captura, rendimiento y errores. De esta forma, los consumidores no acceden directamente al receptor, al buffer o al núcleo DSP, sino a una representación coherente del estado global del backend.
...
Las métricas de recepción proceden de \texttt{EEGReceiver.get\_window\_metrics()} e incluyen tasas, pérdidas estimadas, bloques malformados, estados ADS1299 inválidos, ocupación de cola y descartes. Son las variables que permiten comprobar si el contrato MCU--Python se mantiene estable durante la ejecución.

Como se indicó al describir las cadencias del backend, \texttt{SNAPSHOT\_PUBLISH\_PERIOD\_SEC=0.2} fija una publicación aproximada a \SI{5}{\hertz}. Esa frecuencia pertenece a la vista pública del estado, no al cálculo espectral ni a la generación musical.
