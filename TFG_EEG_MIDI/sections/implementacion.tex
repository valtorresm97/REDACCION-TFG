...
La recepción en Python comienza en la frontera definida con el firmware. El microcontrolador no envía muestras aisladas, sino bloques completos mediante el evento \texttt{eeg\_block\_uV}. Cada bloque contiene 8 muestras de 4 canales, por lo que, con una frecuencia de muestreo de \SI{250}{\hertz}, representa aproximadamente \SI{32}{\milli\second} de señal EEG.

El formato de este evento se centraliza en el módulo \texttt{eeg\_contract.py}. Este archivo actúa como contrato común entre firmware, backend, capturas y herramientas de análisis: fija la frecuencia de muestreo, el número de canales, el tamaño de bloque y las reglas mínimas para comprobar que el payload recibido puede interpretarse correctamente. De esta forma, si el firmware o las herramientas de análisis cambian el formato de los datos, el punto de validación queda localizado en un único módulo.

La entrada real al backend se realiza mediante el método \texttt{EEGReceiver.eeg\_block\_uV()}. Este método funciona como \textit{callback}: se ejecuta automáticamente cuando llega un nuevo bloque desde Bridge. Su función se mantiene deliberadamente ligera. No calcula características espectrales ni genera música, sino que comprueba que el bloque recibido es coherente y lo prepara para ser procesado después por el resto del backend.

Durante esta validación, el receptor comprueba tres aspectos principales: que el payload tiene la estructura esperada, que los índices de bloque y de muestra mantienen continuidad temporal, y que el campo de estado del ADS1299 presenta el prefijo válido. Si se detectan problemas, el backend actualiza contadores de diagnóstico, como bloques malformados, pérdidas estimadas o estados inválidos. Si el bloque es correcto, se encapsula como \texttt{BlockItem} y se introduce en una cola interna.

Esta cola es importante porque desacopla la recepción de datos del procesamiento digital. El \textit{callback} solo valida y encola; posteriormente, en el ciclo normal de ejecución, el backend vacía la cola mediante \texttt{drain\_blocks\_to\_processor()} y entrega los bloques completos a \texttt{EEGSignalProcessor.add\_block\_uV()}. Con esta separación, una operación más costosa, como el cálculo espectral, no bloquea la llegada de nuevos eventos desde el firmware.
...
\begin{table}[H]
    ...
    \begin{tabular}{p{0.30\textwidth}p{0.20\textwidth}p{0.40\textwidth}}
        ...
        Duración del hop & \SI{256}{\milli\second} & Periodicidad nominal de actualización de características EEG. \\
        Snapshot & \SI{0.2}{\second} & Publicación lógica del estado a \SI{5}{\hertz}; no marca la cadencia de generación musical. \\
        \hline
    \end{tabular}
\end{table}

Conviene distinguir estas cadencias dentro del backend. La recepción de bloques EEG se produce cada \SI{32}{\milli\second}, ya que cada evento contiene 8 muestras a \SI{250}{\hertz}. El cálculo de características espectrales se actualiza cada \texttt{FEATURE\_HOP\_SAMPLES=64} muestras, es decir, cada \SI{256}{\milli\second}, aproximadamente a \SI{3.9}{\hertz}. En cambio, el \textit{snapshot} se reconstruye cada \SI{0.2}{\second}, a \SI{5}{\hertz}, y actúa como vista pública del estado del backend para WebUI, capturas y validación. Esta frecuencia de publicación no implica que se genere música nueva a \SI{5}{\hertz}; la generación musical sigue una cadencia propia, asociada a la duración del compás musical.
...
La base matemática del análisis espectral, la densidad espectral de potencia, el cálculo de potencia por bandas y el método multitaper se ha introducido en el apartado de Conceptos previos. En esta sección se documenta cómo se integra ese análisis dentro del backend Python.
...
\begin{figure}[H]
    ...
    \IfFileExists{images/uml/impl_backend_snapshot_schema.png}{%
        \includegraphics[width=0.98\textwidth]{images/uml/impl_backend_snapshot_schema.png}
    }{%
...
El \textit{snapshot} se publica con \texttt{SNAPSHOT\_PUBLISH\_PERIOD\_SEC=0.2}, aproximadamente a \SI{5}{\hertz}. Esta frecuencia corresponde a la actualización de la vista pública del backend para monitorización, visualización y validación. No debe confundirse con la cadencia del cálculo espectral, que viene dada por el \textit{hop} de 64 muestras, ni con la cadencia de generación musical, que se define en la etapa de sonificación.
