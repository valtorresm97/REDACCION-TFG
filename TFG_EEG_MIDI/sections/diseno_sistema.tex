...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[
        ...
        \node[signal] (frame)     at (10.2,-2.25) {status 24 bits\\CH1--CH4 signed 24 bits};
        \node[signal] (escala)    at (6.8,-2.25) {validación de status\\escalado \texttt{LSB\_V}};
        \node[signal] (filtros)   at (3.4,-2.25) {filtros MCU\\HP + notch 50 Hz + LP};
        \node[signal] (uv)        at (0,-2.25) {muestras EEG\\en \si{\micro\volt}};
        ...
        \node[note] at (3.4,-0.85) {CH1 es el canal EEG principal\\en la configuración final};
        \node[note] at (6.8,-3.15) {CH2--CH4 se conservan\\por contrato de datos};
    \end{tikzpicture}
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[
        ...
        payloadblock/.style={draw, rounded corners, align=center, text width=5.35cm, minimum height=1.1cm, font=\footnotesize, fill=gray!8},
        ...
        \node[commblock]    (ring)    at (0,-1.35) {\texttt{TxBlockRing}\\agrupa 8 muestras};
        \node[payloadblock] (notify)  at (0,-2.95) {\texttt{Bridge.notify(\"eeg\_block\_uV\")}\\\texttt{block\_idx}, \texttt{first\_sample\_idx}, \texttt{sample\_count}\\\texttt{status + ch1..ch4} en \si{\micro\volt}};
        \node[commblock]    (rx)      at (0,-4.65) {\texttt{EEGReceiver}\\validación de contrato};
        \node[commblock]    (queue)   at (0,-6.0) {cola RX\\bloques válidos};
        \node[commblock]    (backend) at (0,-7.35) {\texttt{BackendService}\\drenado periódico};
        \node[commblock]    (proc)    at (0,-8.7) {\texttt{EEGSignalProcessor}\\buffer multicanal};
        \node[auxblock]     (capture) at (5.0,-6.0) {\texttt{CaptureManager}\\ruta lateral};
        ...
        \node[note] at (-4.2,-1.35) {Adquisición a \SI{250}{\hertz}\\31,25 bloques/s esperados};
        \node[note] at (4.2,-4.65) {Comprueba longitud,\\índices y status ADS1299};
        \node[note] at (4.2,-8.7) {Salida del bloque:\\muestras listas para DSP};
    \end{tikzpicture}
