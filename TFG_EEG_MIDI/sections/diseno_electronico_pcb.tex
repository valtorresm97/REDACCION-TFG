% !TeX root = ../main.tex
% Este subfile desarrolla la subsección "Diseño electrónico y PCB" declarada al final de sections/diseno_sistema.tex.
...
En el presente TFG no se reproduce literalmente aquella arquitectura, sino que se toma como base conceptual para adaptarla a un objetivo diferente: transformar una señal EEG real en eventos musicales enviados por una salida MIDI física. Por tanto, el diseño electrónico no se entiende únicamente como una placa de adquisición, sino como el soporte físico de una cadena EEG--MIDI completa. Esta diferencia condiciona la elección del \textit{front-end}, la plataforma de control, la organización de la comunicación y la incorporación de una etapa final de salida MIDI.
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[
        block/.style={draw, rounded corners, align=center, minimum width=3.2cm, minimum height=0.95cm, font=\footnotesize},
        newblock/.style={draw, rounded corners, align=center, minimum width=3.2cm, minimum height=0.95cm, font=\footnotesize, fill=gray!8},
        arrow/.style={-{Latex[length=3mm]}, thick},
        note/.style={align=center, font=\scriptsize}
    ]
        \node[block] (base1) at (0,1.5) {Electrodos\biopotenciales};
        \node[block] (base2) at (3.8,1.5) {AFE biomédico\diseño base};
        \node[block] (base3) at (7.6,1.5) {Microcontrolador\comunicación};
        \node[block] (base4) at (11.4,1.5) {Datos de\adquisición};

        \node[newblock] (new1) at (0,-1.5) {Electrodos\EEG};
        \node[newblock] (new2) at (3.8,-1.5) {ADS1299--4PAGR\4 canales, 24 bits};
        \node[newblock] (new3) at (7.6,-1.5) {Arduino UNO Q\MCU + Linux};
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[
        ...
        \node[block] (j1) at (0,0) {Conectores\electrodos};
        ...
        \node[block] (uno) at (14.8,0) {Arduino UNO Q\MCU + Linux};
        \node[block] (midi) at (18.5,0) {DIN-5\MIDI OUT};

        \node[isol] (power) at (7.4,-2.3) {Alimentación aislada\CRE1S0505SC + reguladores};
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[
        side/.style={draw, rounded corners, align=center, text width=3.6cm, minimum height=1.0cm, font=\footnotesize},
        isol/.style={draw, rounded corners, align=center, text width=3.0cm, minimum height=4.9cm, font=\footnotesize, fill=gray!10},
        arrow/.style={-{Latex[length=3mm]}, thick},
        retarrow/.style={{Latex[length=3mm]}-{Latex[length=3mm]}, thick},
        sig/.style={font=\scriptsize}
    ]
        \node[side] (uno) at (0,0) {Arduino UNO Q\lado no aislado};
        \node[isol] (iso) at (4.6,0) {ADuM3151\aislamiento digital};
        \node[side] (ads) at (9.2,0) {ADS1299--4PAGR\lado aislado};

        \draw[arrow] (uno.east) ++(0,1.8) -- node[above, sig]{\texttt{SPI1\_SCLK} / \texttt{SPI\_SCLK\_ISO}} (ads.west |- 0,1.8);
        \draw[arrow] (uno.east) ++(0,1.1) -- node[above, sig]{\texttt{SPI1\_MOSI} / \texttt{SPI\_MOSI\_ISO}} (ads.west |- 0,1.1);
        \draw[arrow] (ads.west) ++(0,0.4) -- node[above, sig]{\texttt{SPI\_MISO\_ISO} / \texttt{SPI1\_MISO}} (uno.east |- 0,0.4);
        \draw[arrow] (uno.east) ++(0,-0.3) -- node[above, sig]{\texttt{FE\_SPI\_CS} / \texttt{SPI\_CS\_ISO}} (ads.west |- 0,-0.3);
        \draw[arrow] (uno.east) ++(0,-1.0) -- node[above, sig]{\texttt{FE\_START} / \texttt{START\_ISO}} (ads.west |- 0,-1.0);
        \draw[arrow] (uno.east) ++(0,-1.7) -- node[above, sig]{\texttt{FE\_RESET} / \texttt{RESET\_ISO}} (ads.west |- 0,-1.7);
        \draw[arrow] (ads.west) ++(0,-2.4) -- node[below, sig]{\texttt{DRDY\_ISO} / \texttt{FE\_DRDY}} (uno.east |- 0,-2.4);
    \end{tikzpicture}}
    \caption{Comunicación SPI aislada entre Arduino UNO Q y ADS1299--4PAGR. El ADuM3151 separa el lado de control del lado de adquisición, manteniendo las señales SPI y de control necesarias para la lectura continua del conversor.}
