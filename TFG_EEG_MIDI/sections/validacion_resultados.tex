...
En el firmware se conservaron las métricas impresas por el Monitor/App Lab mediante bloques \texttt{[BENCH] EEG\_MIDI}. Para no alterar el transporte real, no se añadió ningún evento \texttt{Bridge.notify} adicional; el log del monitor se copió y se parseó después con una herramienta específica. Las métricas principales se resumen en la Tabla~\ref{tab:validacion_bench_mcu}.

\begin{table}[htbp]
    \centering
    \caption{Resumen de benchmarks del firmware/MCU.}
    \label{tab:validacion_bench_mcu}
    \begin{tabular}{p{0.32\textwidth} c c p{0.29\textwidth}}
        \hline
        \textbf{Métrica MCU} & \textbf{Valor} & \textbf{Referencia} & \textbf{Lectura} \\
        \hline
        \texttt{filt\_avg\_us} mediano & \SI{5.050}{\micro\second} & \SI{4}{\milli\second}/muestra & Filtrado despreciable frente al periodo de muestreo. \\
        \texttt{notify\_avg\_us} mediano & \SI{3367.250}{\micro\second} & \SI{32}{\milli\second}/bloque & Publicación Bridge asumible. \\
        \texttt{notify\_max\_us\_win} máximo & \SI{11528}{\micro\second} & \SI{32}{\milli\second}/bloque & Pico inferior al periodo de bloque. \\
        \texttt{loop\_max\_us\_win} máximo & \SI{12886}{\micro\second} & \SI{32}{\milli\second}/bloque & Bucle con margen en las ventanas analizadas. \\
        \texttt{qmax\_global} & 1 & Cola estable & Sin acumulación significativa de bloques. \\
        \texttt{drops\_total} & 0 & 0 esperado & Sin pérdidas por cola TX. \\
        \hline
    \end{tabular}
\end{table}

En el MCU, el coste dominante no fue el filtrado, sino la publicación de bloques por Bridge. Aun así, el máximo de \texttt{notify} por ventana fue de \SI{11.528}{\milli\second} y el máximo de \texttt{loop} por ventana fue de \SI{12.886}{\milli\second}, ambos por debajo del periodo de bloque de \SI{32}{\milli\second}. Además, la cola TX permaneció controlada, con \texttt{qmax\_global = 1} y \texttt{drops\_total = 0}.
