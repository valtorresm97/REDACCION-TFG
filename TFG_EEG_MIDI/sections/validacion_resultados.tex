...
\begin{table}[H]
    ...
    \begin{tabular}{p{0.14\textwidth} p{0.42\textwidth} p{0.36\textwidth}}
        ...
        \hline
        Delta &
        Presenta una contribución relevante en varias capturas, pero es sensible a deriva lenta, movimiento y transitorios de contacto. &
        Se utiliza como información contextual de baja frecuencia, no como indicador fisiológico aislado. \\

        Theta &
        Se obtiene de forma estable como parte del perfil espectral, aunque no mostró una separación clara entre condiciones con el protocolo disponible. &
        Se conserva como modulador secundario dentro del conjunto de bandas, sin asociarla a un estado cognitivo concreto. \\

        Alpha &
        Fue la banda más interpretable en el montaje ear-EEG: aumentó en ojos cerrados respecto a ojos abiertos y mejoró la relación alpha/beta. &
        Se usa como modulador principal de reposo relativo mediante variables como \texttt{alpha\_drive} y \texttt{alpha\_stability}. \\

        Beta &
        Aporta información de actividad rápida, pero puede mezclarse con actividad muscular, movimiento o tensión facial. &
        Se combina con RMS, gamma y calidad de ventana para generar controles de actividad/tensión musical, evitando interpretarla de forma aislada. \\

        Gamma &
        Es la banda más sensible a EMG y artefactos de movimiento en el montaje experimental empleado. &
        No se usa como marcador EEG directo; se trata como componente de tensión o posible artefacto, siempre condicionada por el \textit{quality gate}. \\
        \hline
    \end{tabular}
\end{table}
```
